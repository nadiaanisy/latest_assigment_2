...
export function ProductSlider({ addToCart }: ProductSliderProps) {
  const {
    t
  } = useCustomHook();
  ...
  const nextSlide = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 1) % products_list.length)
  }

  const prevSlide = () => {
    setCurrentIndex((prevIndex) => (prevIndex - 1 + products_list.length) % products_list.length)
  }
  ...
  const currentProduct = products_list[currentIndex];
    ...
    <div className="relative">
      ...
      <div className="text-center mt-2">
        <p className="text-sm text-muted-foreground">
          {currentIndex + 1} {t('OF')} {products_list.length}
        </p>
      </div>
    </div>
  )
